...
本报告对 KYN28-12 型高压开关柜（1250A 进线柜和 4000A 出线柜）内部电场分布进行有限元仿真分析。通过 ANSYS Maxwell 静电场求解器计算柜内各关键部位的电场强度分布，评估电气间隙和爬电距离的合理性，为开关柜的绝缘设计提供理论依据。

#block(fill: rgb("#e3f2fd"), inset: 10pt, radius: 4pt, width: 100%)[
  *分析目的*：评估开关柜内关键部位的电场强度，识别可能发生电晕或击穿的高场强区域，指导绝缘结构优化设计。
  
  *分析对象*：
  - 1250A 进线柜（KYN28-12/1250A）
  - 4000A 出线柜（KYN28-12/4000A）
]
...
仿真模型基于 KYN28-12/1250A 型进线柜的三维几何模型，包括：
- 主母排（三相 A/B/C）
- 绝缘支撑件（环氧树脂绝缘子）
- 金属框架和隔板
- 接地外壳
...
仿真模型基于 KYN28-12/4000A 型出线柜的三维几何模型，结构与进线柜类似。
...
#block(fill: rgb("#fff8e1"), inset: 10pt, radius: 4pt, width: 100%)[
  *说明*：导体材料在静电场分析中视为等电位体，不需要设置介电常数。绝缘材料的介电常数影响电场分布，环氧树脂绝缘子采用 εr = 3.8。
]
...
直流传导场计算方程：$ bold(E) = -nabla V $

#block(fill: rgb("#fff8e1"), inset: 10pt, radius: 4pt, width: 100%)[
  *边界条件设置*：
  - 将载流导体表面施加电势 *59397V*（注：工频耐受电压 $times sqrt(2)$）
  - 将所有接地的金属外壳及金属隔板表面施加电势 *0V*
  - 将真空灭弧室内屏蔽环设置为 *悬浮电位*（电荷 $Q_0 = 0 "C"$，电压初始值 $V_"init" = 0 "V"$）
  - 将绝缘介质（空气、陶瓷、环氧树脂等）设置为电荷初始值 0C、电势初始值 0V
]
...
#grid(
  ...
  figure(
    image("../field_plots/ElectrostaticField/1250AIn/GeometricModel.png", height: 8cm),
    caption: [1250A 进线柜三维几何模型]
  ),
  figure(
    image("../field_plots/ElectrostaticField/4000AOut/GeometricModel.png", height: 8cm),
    caption: [4000A 出线柜三维几何模型]
  )
)
...
#grid(
  ...
  column-gutter: 1em,
  figure(
    image("../field_plots/ElectrostaticField/1250AIn/Mesh.png", width: 100%),
    caption: [1250A 进线柜网格划分]
  ),
  figure(
    image("../field_plots/ElectrostaticField/4000AOut/Mesh.png", width: 100%),
    caption: [4000A 出线柜网格划分]
  )
)
...
#grid(
  ...
  align: (center + horizon, center + horizon),
  figure(
    image("../field_plots/ElectrostaticField/1250AIn/MeshCount.png", height: 6cm),
    caption: [1250A 进线柜网格统计]
  ),
  figure(
    image("../field_plots/ElectrostaticField/4000AOut/MeshCount.png", height: 6cm),
    caption: [4000A 出线柜网格统计]
  )
)

网格划分采用自适应网格加密技术，在高场强区域（如母排边缘、触头区域）进行局部加密。1250A 进线柜生成约 1088 万个单元，4000A 出线柜生成约 2375 万个单元。
...
#grid(
  ...
  align: (center + horizon, center + horizon),
  figure(
    image("../field_plots/ElectrostaticField/1250AIn/Voltage_Distribution.png", height: 8cm),
    caption: [1250A 进线柜电压分布]
  ),
  figure(
    image("../field_plots/ElectrostaticField/4000AOut/Voltage_Distribution.png", height: 8cm),
    caption: [4000A 出线柜电压分布]
  )
)

电压分布云图显示：
- 最大电压约 59.5 kV（对应工频耐压试验电压峰值）
- 三相导体呈现明显的电压梯度分布
- 金属框架和外壳保持接地电位（0 V）
- 两个柜型的电压分布规律一致
...
#grid(
  ...
  column-gutter: 1em,
  figure(
    image("../field_plots/ElectrostaticField/1250AIn/E_Field1.png", width: 100%),
    caption: [1250A 进线柜电场分布（整体）]
  ),
  figure(
    image("../field_plots/ElectrostaticField/1250AIn/E_Field2.png", width: 100%),
    caption: [1250A 进线柜电场分布（局部）]
  )
)
...
#grid(
  ...
  column-gutter: 1em,
  figure(
    image("../field_plots/ElectrostaticField/4000AOut/E_Field1.png", width: 100%),
    caption: [4000A 出线柜电场分布（整体）]
  ),
  figure(
    image("../field_plots/ElectrostaticField/4000AOut/E_Field2.png", width: 100%),
    caption: [4000A 出线柜电场分布（带网格）]
  )
)
...
#figure(
  ...
  caption: [4000A 出线柜关键区域电场强度统计]
)

#block(fill: rgb("#e8f5e9"), inset: 10pt, radius: 4pt, width: 100%)[
  *关键发现*：
  - 两个柜型所有监测区域的电场强度均低于安全阈值，安全系数均大于 1.2
  - 1250A 进线柜的母排边缘场强 1.25 kV/mm，安全系数较低（1.20），建议进行倒圆处理
  - 4000A 出线柜的整体场强低于进线柜，绝缘裕度更充足
]
...
+ 1250A 进线柜相间最大电场强度约 *0.92 kV/mm*，对地最大电场强度约 *0.52 kV/mm*，均有足够的安全裕度
+ 4000A 出线柜整体场强更低，相间最大场强约 *0.72 kV/mm*，安全系数更高
+ 触头间隙区域场强最高，1250A 进线柜约 *1.74 kV/mm*，但仍低于击穿阈值，安全系数为 1.72
+ 母排边缘存在场强集中现象，最大场强 *1.25 kV/mm*，建议进行边缘倒圆优化
+ 绝缘子表面场强较低（≤ 0.45 kV/mm），沿面闪络风险可控
+ 整体绝缘设计满足 GB 3906《3.6 kV～40.5 kV 交流金属封闭开关设备和控制设备》标准要求

#block(fill: rgb("#e3f2fd"), inset: 10pt, radius: 4pt, width: 100%)[
  *总体评价*：KYN28-12 型开关柜（含 1250A 进线柜与 4000A 出线柜）的绝缘设计合理，电气间隙和爬电距离配置满足标准要求。建议对母排边缘进行倒圆处理，并在高场强区域加强绝缘监测。
]
